...
async function loadSlokas() {
  ...
  if (chapterId) {
    const chapterSnap = await getDoc(doc(db, "slokaChapters", chapterId));

    if (chapterSnap.exists()) {
      chapterTitle.innerText = chapterSnap.data().title;
      chapterIds = [chapterId];
    } else {
      chapterTitle.innerText = "Chapter Not Found";
      return;
    }

  } else {
    const categorySnap = await getDoc(doc(db, "slokaCategories", categoryId));

    if (categorySnap.exists()) {
      chapterTitle.innerText = categorySnap.data().title;
    }

    const chaptersSnapshot = await getDocs(collection(db, "slokaChapters"));
  ...
  const slokasSnapshot = await getDocs(collection(db, "slokas"));
